...
export async function GET(request: NextRequest) {
  try {
    ...
    const [surveysRes, sessionsRes, readsRes] = await Promise.all([
      ...
      (supabase as any).from('paper_read_progress').select('created_at').gte('created_at', sinceStr),
    ]);

    // Build date map for last N days
    const dateMap: Record<string, { surveys: number; reads: number; new_sessions: number }> = {};
    for (let i = 0; i < days; i++) {
      const d = new Date();
      d.setDate(d.getDate() - i);
      const key = d.toISOString().split('T')[0];
      dateMap[key] = { surveys: 0, reads: 0, new_sessions: 0 };
    }
    ...
    for (const r of (readsRes.data || [])) {
      ...
      if (key && dateMap[key]) dateMap[key].reads++;
    }
